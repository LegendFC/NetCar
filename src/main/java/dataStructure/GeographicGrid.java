...
public class GeographicGrid {
	...
	public GeographicGrid(LonLat lonLatGCJ) {
		lonLat = lonLatGCJ;
		double lonCode_Double = lonLat.getLongtitude() - 60;
		double latCode_Double = lonLat.getLatitude() * 1.5;
	...
	/**
	 * 传入地理网格号码，以及网格的x2和y2坐标 解析经纬度等。此处给出的经纬度是网格左上角的。
	 * 
	 * @param code
	 */
	...
	/**
	 * 不是纯粹的重写toString方法。此处的输出为，网格号、左下角经纬度、右上角经纬度
	 * @return 网格号、左下角经纬度、右上角经纬度
	 */
	public String toStringGeographicCodeLonLat1LonLat2() {
		StringBuilder strBuilder = new StringBuilder(geographicCode);
		strBuilder.append(",").append(lonLat.toString()).append(",").append(String.valueOf(lonLat.getLongtitude() + 0.0125)).append(",")
				.append(String.valueOf(lonLat.getLatitude() + 0.0125 * 2.0 / 3.0));
		return strBuilder.toString();
	}
	
	/**
	 * 不是纯粹的重写toString方法。此处的输出为，左下角经纬度、右上角经纬度
	 * @return 左下角经纬度、右上角经纬度
	 */
	public String toStringLonLat1LonLat2() {
		StringBuilder strBuilder = new StringBuilder(lonLat.toString());
		strBuilder.append(",").append(String.valueOf(lonLat.getLongtitude() + 0.0125)).append(",")
				.append(String.valueOf(lonLat.getLatitude() + 0.0125 * 2.0 / 3.0));
		return strBuilder.toString();
	}
	...
	public static void main(String[] args) {
		GeographicGrid test = new GeographicGrid(new LonLat("116.1772382941725 40.154219108081506"));
